...
void DirectSail_ShowStormPrompt()
{
    totalInfo = "Storm diverter warns - storm is directly ahead!";
    SetFormatedText("INFO_TEXT", totalInfo);
    SendMessage(&GameInterface,"lsl",MSG_INTERFACE_MSG_TO_NODE,"INFO_TEXT",5);
    SetFormatedText("INFO_TEXT_QUESTION", "Avoid storm zone?");
...
void DirectSail_StormPromptAvoid()
{
    ...
    if(sStormPromptPath != "" && CheckAttribute(&worldMap, sStormPromptPath))
    {
        worldMap.(sStormPromptPath).needDelete = "Storm diverter avoid";
    }

    trace("DS STORM DIVERTER: avoid path=" + sStormPromptPath);
...
void DirectSail_StormPromptAgainst()
{
    ...
    if(sStormPromptPath != "" && CheckAttribute(&worldMap, sStormPromptPath))
    {
        worldMap.(sStormPromptPath).needDelete = "Reload delete storm encounter";
    }
    ...
    trace("DS STORM DIVERTER: against path=" + sStormPromptPath +
          " tornado=" + bStormPromptTornado);
    bStormPromptResolved = true;
    IDoExit(RC_INTERFACE_MAP_EXIT);
}
...
void evtDirSailStorm(string sStormPath, bool bIsTornado)
{
    bStormPrompt = true;
    bStormPromptResolved = false;
    bStormPromptTornado = bIsTornado;
    sStormPromptPath = sStormPath;
    SetNodeUsing("B_OK", true);
    SetNodeUsing("B_CANCEL", true);
}
//extern call from Islands_loader.c
